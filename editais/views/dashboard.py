...
def dashboard_home(request: HttpRequest) -> HttpResponse:
    ...
    if request.user.is_staff:
        ...
        recent_editais = (
            ...
            .filter(data_atualizacao__gte=timezone.now() - timedelta(days=7))
        ...
        recent_startups = (
            ...
            .select_related("proponente", "edital")
            .order_by("-data_atualizacao")[:5]
        )
        ...
        context = {
            ...
            "startups_incubadas": Startup.objects.filter(
        ...
        user_startups = Startup.objects.filter(proponente=request.user).order_by(
            "-data_atualizacao"
        )[:5]
...
def dashboard_editais(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        if search_query:
            # Use model's search method (PostgreSQL full-text search)
        ...
        # Use subquery to keep filtering in the database (avoids materializing IDs in Python)
        total_submissoes = Startup.objects.filter(
            proponente__isnull=False,
            edital_id__in=stats_base.values("id"),
        ).count()
    ...
    try:
        ...
        if status_filter:
            status_value = status_mapping.get(
                status_filter.lower(), status_filter.lower()
            )
        ...
        available_editais = (
            Edital.objects.select_related("created_by", "updated_by")
            .only("id", "titulo", "data_atualizacao", "created_by", "updated_by")
...
def dashboard_submeter_startup(
    request: HttpRequest,
...
def admin_dashboard(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        if not editais_proximos_prazo:
            ...
            if use_cache:
                cache.set(
                    cache_key_deadline, editais_proximos_prazo, CACHE_TTL_5_MINUTES
                )
        ...
        top_entidades = (
            Edital.objects.exclude(entidade_principal__isnull=True)
            .exclude(entidade_principal="")
            .values("entidade_principal")
            .annotate(count=Count("id"))
            .order_by("-count")[:10]
        )
