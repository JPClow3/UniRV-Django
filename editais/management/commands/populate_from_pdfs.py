...
class Command(BaseCommand):
    ...
    def extract_info_from_filename(self, filename):
        ...
        numero_match = re.search(
            r"(\d+[/_]\d+|N[°º]?\s*(\d+)|Edital[_\s]*(\d+))", name, re.IGNORECASE
        )
        numero_edital = None
        if numero_match:
            numero_edital = (
                numero_match.group(0)
                .replace("_", "/")
                .replace("N°", "")
                .replace("Nº", "")
                .strip()
        ...
        titulo = re.sub(
            r"^(EDITAL|CHAMADA|Edital|Chamada)[_\s]*", "", titulo, flags=re.IGNORECASE
        )
        titulo = re.sub(
            r"___+", " - ", titulo
        )  # Replace multiple underscores with dash
    ...
    def handle(self, *args, **options):
        ...
        if not pdf_files:
            self.stdout.write(
                self.style.WARNING(f"No PDF files found in '{base_path}'.")
            )
            return
...
<li>Pontos específicos para a UniRV</li>
            ...
            if created:
                ...
                updated = False
                if (
                    not edital.numero_edital or edital.numero_edital == ""
                ) and info.get("numero_edital"):
                    edital.numero_edital = info["numero_edital"]
                    updated = True
                if (
                    not edital.entidade_principal or edital.entidade_principal == ""
                ) and info.get("entidade_principal"):
                ...
                if updated:
                    ...
                    try:
                        ...
                        updated_count += 1
                        self.stdout.write(
                            self.style.WARNING(f"↻ Atualizado: {info['titulo']}")
                        )
                    except Exception as e:
                        self.stdout.write(
                            self.style.ERROR(
                                f"✗ Erro ao atualizar {info['titulo']}: {e}"
                            )
                        )
                else:
                    self.stdout.write(
                        self.style.WARNING(f"- Já existe: {info['titulo']}")
                    )
