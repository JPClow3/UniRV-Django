...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not no_auth and (
                ...
                self.stdout.write(
                    self.style.SUCCESS("Getting authentication cookie...")
                )
                auth_cookie = self._get_auth_cookie()
                if auth_cookie:
                    self.stdout.write(
                        self.style.SUCCESS("Authentication cookie obtained")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            "Failed to get auth cookie, some pages may fail"
                        )
                    )
            ...
            if not no_server:
                self.stdout.write(
                    self.style.SUCCESS(f"Starting Django server on port {port}...")
                )
                ...
                self.stdout.write(
                    self.style.WARNING("Skipping server start (--no-server flag set)")
                )
            ...
            if skip_assertions:
                self.stdout.write(
                    self.style.WARNING(
                        "Assertions will be skipped (--skip-assertions flag set)"
                    )
                )
    ...
    def _get_all_urls(self, port):
        ...
        try:
            editais = Edital.objects.filter(slug__isnull=False).exclude(slug="")[
                :10
    ...
    def _get_auth_cookie(self):
        ...
        try:
            ...
            if not user:
                ...
                self.stdout.write(
                    self.style.WARNING("Created test superuser: lighthouse_test")
                )
            ...
            if password:
                ...
                login_success = client.login(
                    username=user.username, password="lighthouse_test_password_123"
                )
    ...
    def _run_lighthouse(
        ...
        try:
            ...
            if not skip_assertions:
                ...
                config_file.write(
                    "        'categories:performance': ['error', { minScore: parseFloat(process.env.LHCI_PERFORMANCE_THRESHOLD || '0.80') }],\n"
                )
                config_file.write(
                    "        'categories:accessibility': ['error', { minScore: parseFloat(process.env.LHCI_ACCESSIBILITY_THRESHOLD || '0.90') }],\n"
                )
                config_file.write(
                    "        'categories:best-practices': ['error', { minScore: parseFloat(process.env.LHCI_BEST_PRACTICES_THRESHOLD || '0.90') }],\n"
                )
                config_file.write(
                    "        'categories:seo': ['error', { minScore: parseFloat(process.env.LHCI_SEO_THRESHOLD || '0.90') }],\n"
                )
                config_file.write("      },\n")
                config_file.write("    },\n")
            ...
            env["LIGHTHOUSE_TIMEOUT"] = (
                "180000"  # 180 seconds for Lighthouse operations
            )
            ...
            self.stdout.write(
                self.style.ERROR(
                    "Lighthouse CI not found. Please install it:\n"
            ...
            self.stdout.write(
                self.style.ERROR(f"Error running Lighthouse CI: {str(e)}")
            )
    ...
    def _cleanup(self):
        ...
        if self.server_process:
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Error stopping server: {str(e)}")
                )
