...
class StartupShowcaseViewTest(TestCase):
    ...
    def test_startup_showcase_loads(self):
        ...
        self.assertTemplateUsed(response, "startups/index.html")
    ...
    def test_category_filtering(self):
        ...
        response = self.client.get(
            reverse("startups_showcase"), {"category": "biotech"}
        )
        self.assertContains(response, self.project2.name)
    ...
    def test_search_functionality(self):
        ...
        response = self.client.get(
            reverse("startups_showcase"), {"search": "biotechnology"}
        )
        self.assertContains(response, self.project2.name)
    ...
    def test_search_with_special_characters(self):
        ...
        response = self.client.get(
            reverse("startups_showcase"), {"search": "!@#$%^&*()"}
        )
        self.assertEqual(response.status_code, 200)
        ...
        if len(startups) > 1:
            for i in range(len(startups) - 1):
                self.assertGreaterEqual(
                    startups[i].submitted_on, startups[i + 1].submitted_on
                )

    def test_database_error_handling(self):
        """Test that database errors return 503 instead of empty 200"""
        with patch(
            "editais.views.public.Startup.objects.select_related"
        ) as mock_select:
    ...
    def test_stats_aggregation_error_handling(self):
        ...
        mock_objects.select_related.return_value.filter.return_value.only.return_value = (
            base
        )
    ...
    def test_xss_in_project_description(self):
        ...
        has_full_escaped_script = (
            "&lt;script&gt;alert(&quot;XSS&quot;)&lt;/script&gt;" in content
        )
        self.assertTrue(
            has_escaped_alert or has_full_escaped_script,
    ...
    def test_complete_filter_search_pagination_workflow(self):
        ...
        response = self.client.get(
            reverse("startups_showcase"), {"search": "Startup 1"}
        )
        self.assertEqual(response.status_code, 200)
        context = response.context
        ...
        found_match = False
        for startup in context["startups"]:
            if "Startup 1" in startup.name or "Startup 1" in (
                startup.description or ""
            ):
                found_match = True
                break
        self.assertTrue(
            found_match or len(context["startups"]) == 0,
            "Search should find matching startups or return empty if no match",
        )

        # 4. Test combined filter and search
        response = self.client.get(
            reverse("startups_showcase"), {"category": "biotech", "search": "Startup"}
        )
        self.assertEqual(response.status_code, 200)
        context = response.context
        ...
        response_all_categories = self.client.get(
            reverse("startups_showcase"), {"category": "all"}
        )
