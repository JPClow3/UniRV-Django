...
def compare_scores(current: Dict, previous: Optional[Dict]) -> Dict:
    ...
    for key in ["performance", "accessibility", "best_practices", "seo", "average"]:
        ...
        comparison[key] = {"change": round(change, 1), "status": status}
...
def generate_trend_report(history: List[Dict], limit: int = 10) -> str:
    ...
    report_lines = [f"Lighthouse Score Trends (Last {len(recent)} runs)", "=" * 60, ""]
...
def main():
    ...
    history_file = (
        project_root / "lighthouse_reports" / "lighthouse_scores_history.json"
    )
    ...
    if previous_scores:
        ...
        for key, comp in comparison.items():
            if key != "average":  # Skip average in detailed comparison
                change_str = (
                    f"+{comp['change']}" if comp["change"] > 0 else str(comp["change"])
                )
                status_emoji = (
                    "📈"
